...
export var renderTasks = function renderTasks() {
  listElem.innerHTML = '';
  getTaskList().then(function (tasksList) {
    return tasksList.sort(compareTasks);
  }).then(function (tasksList) {
    return tasksList.map(createListItem);
  }).then(function (tasksElems) {
    return listElem.append.apply(listElem, _toConsumableArray(tasksElems));
  });
};